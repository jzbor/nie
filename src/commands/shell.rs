...
impl super::Command for ShellCommand {
    fn exec(self) -> NieResult<()> {
        let repo_refs = self.specs.iter().map(|s| s.repository()).cloned();
        let filenames = self.specs.iter().map(|s| s.filename().cloned());
        let attributes = self.specs.iter().map(|s| s.attribute()).cloned();
